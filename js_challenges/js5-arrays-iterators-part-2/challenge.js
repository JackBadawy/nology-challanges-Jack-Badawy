...
export const totalScoresArr = (scoresArr) => {
    const totalScores = scoresArr.reduce((acc, cur) => acc + cur, 0);
    return totalScores;
};
...
export const reverseString = (toReverse) => {
    ...
    // return toReverse.split("").reduce((acc, char) => {
    //     return char + acc;
    // }, "");
...
export const sortCharactersAlphabetically = (charcterArr) => {
    return charcterArr // ["X", "B", "B", "b", "g", "l", "n", "x"]
        .map((char) => char.toLocaleLowerCase()) // ["x", "b", "b", "b", "g", "l", "n", "x"]
        .sort(); // ["b", "b", "b", "g", "l", "n", "x", "x"]
...
export const checkItemInstock = (toCheck) => {
    ...
    // const index = stockList.findIndex((product) => product.startsWith("b"));
...
export const checkPrimaryColours = (coloursArr) => {
    // const colorKey = ["red", "blue", "yellow"];
    // return coloursArr.every((color) => colorKey.includes(color));
    // return coloursArr.every((color) => color.match(/^yellow|red|blue$/).length);
};
...
export const checkStringPalindrome = (stringOne) => {
    // const splitString = stringOne.split("");
    ...
    // while (head < tail) {
    //     if (stringOne[head++] !== stringOne[tail--]) {
    //         return false;
    //     }
    // }
...
export const encryptString = (toEncrypt, bucketsLength = 3) => {
    const buckets = new Array(bucketsLength).fill(null).map(() => []);

    return toEncrypt
        .split("")
        .reduce((acc, char, i) => {
            acc[i % bucketsLength].push(char);
            return acc;
        }, buckets)
        .flat()
        .join("");
