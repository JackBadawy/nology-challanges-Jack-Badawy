...
export const removeEvenNumbers = (numberArr) => {
    // const newNumbers = numberArr.filter((n) => n % 2 !== 0);
    // return newNumbers;
...
export const generateAverage = (numberArr) => {
    ...
    const total = numberArr.reduce((acc, cur) => {
        return acc + cur;
    }, 0);
...
export const generateHighscores = (playersArr, scoresArr) => {
    ...
    //     tmp.push(`P:${i + 1} ${playersArr[i]} scored ${scoresArr[i]}`);
    // }

    return playersArr.map((playerName, i) => {
        return `P:${i + 1} ${playerName} scored ${scoresArr[i]}`;
    });
};
...
export const encryptString = (toEncrypt, buckets = 3) => {
    // e n c r y p t e d
    // 0 1 2 3 4 5 6 7 8
    let lists = new Array(buckets).fill(0).map(() => []);

    if (toEncrypt.length <= buckets) {
        return toEncrypt;
    }

    for (let i = 0; i < toEncrypt.length; ++i) {
        lists[i % buckets].push(toEncrypt[i]);
    }
