...
export const getIsValidOnNameTag = () => {
  if (typeof nameTagOption === "string" && nameTagOption.length <= 8) {
    return true;
  } else {
    return false;
  }
...
export const getHasUppercaseLetters = () => {
  // Write your code here
  if (stringWithUppercaseLetters === stringWithUppercaseLetters.toLowerCase()) {
    return false;
  } else {
    return true;
  }
...
export const convertPascalCaseToSnakeCase = () => {
  // Write your code here
  //my guess here is to break each character into a sring than if string is uppercase add underscore for it exept first iteration
  let stringArray = pascalCaseVariableName.split("");

  for (let i = 0; i < stringArray.length; i++) {
    if (stringArray[i] === stringArray[i].toUpperCase() && i != 0) {
